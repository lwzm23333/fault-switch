...
ADVANCED_IMF_FEATURES = BASIC_IMF_FEATURES + [
    "kurtosis",
    "flatness",
    "roll_off"
]

FULL_IMF_FEATURES = ADVANCED_IMF_FEATURES + [
    # contrast (14)
    ...
    *[f"delta2_mfcc_{i}" for i in range(12)],
    # pitch (3)
    "pitch_mean", "pitch_std", "pitch_strength"
]
...
def extract_imf_features(
    ...
    """
    U: (K, N) IMFs
    return: (K, D) IMF-level feature matrix
    """
    ...
    for k in range(K):
        ...
        feats["roughness"] = float(np.std(u)) if len(u) > 0 else 0.0
        feats["kurtosis"] = float(kurtosis(u)) if len(u) > 0 else 0.0

        # ----- envelope -----
        env = calculate_amplitude_envelope(u)
        feats["env_mean"], feats["env_std"] = env
        ...
        # ----- contrast -----
        contrast = calculate_spectral_contrast(u, fs)
        for i, v in enumerate(contrast):
            feats[f"contrast_{i}"] = float(v)

        # ----- MFCC -----
        mfcc = calculate_mfcc(u, fs)
        for i, v in enumerate(mfcc):
            feats[f"mfcc_{i}"] = float(v)

        d2 = calculate_delta_delta_mfcc(u, fs)
        for i, v in enumerate(d2):
            feats[f"delta2_mfcc_{i}"] = float(v)

        # ----- pitch (safe) -----
        p = calculate_pitch_features(u, fs)
        feats["pitch_mean"] = p[0]
        feats["pitch_std"] = p[1]
        feats["pitch_strength"] = p[2]

        # ----- assemble -----
        token = [feats.get(f, 0.0) for f in feature_set]
        tokens.append(token)

    return np.asarray(tokens, dtype=float)

# 1️⃣ EMD + RF / VMD-fixed + RF（baseline）
# X_imf = extract_imf_features(imfs, fs, BASIC_IMF_FEATURES)
# X = X_imf.flatten()   # 直接拼接

# 2️⃣ VMD-adaptive + RF（无 token）
# X_imf = extract_imf_features(imfs, fs, ADVANCED_IMF_FEATURES)
# X = X_imf.flatten()

# 3️⃣ IMF-token（完整方法）
# tokens = extract_imf_features(imfs, fs, FULL_IMF_FEATURES)
# X = fuse_tokens(tokens)   # 你的融合 / 权重 / attention
