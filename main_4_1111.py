...
def _process_single_segment(args):
    ...
    try:
        ...
        U_fix = U.copy()
        e_fix = e.copy()
        return {"ok": True, "seg_id": seg_id, "U_fix": U_fix, "e_fix": e_fix,
                "K": int(K_used), "alpha": float(alpha_used), "meta": meta,"seg": seg}
    except Exception as exc:
        return {"ok": False, "seg_id": seg_id, "error": str(exc)}
    ...
    for i, (path, y) in enumerate(items, 1):
        ...
        np.savez(
            ...
            seg_vmd_results=np.array([
                dict(seg_id=r["seg_id"], U_fix=r["U_fix"], e_fix=r["e_fix"],
