...
def extract_segment_features(seg, fs, phys_bands):
    ...
    # 5) IMF-token 特征
    tokens = imf_features(U_den, fs, phys_bands)
    #tokens_2 = []
...
def train_model(model_name="svm", cv_folds=None, feature_dir="outputs/files_by_class"):
    ...
    for f in files:
        ...
        all_Z.append(d["fused_features"])     # features_2等文件
        #all_Z.append(d["Z_file"])           # files_by_class_2 等文件类别
        all_Y.append(int(d["labels"]))
    X = np.vstack(all_Z)
    y = np.array(all_Y)
    ...
    results, y_true, y_pred, clf = crossval_train_eval(
        X, y, model_name=model_name, n_splits=cv_folds, return_preds=True
    )
    f1 = results["f1_macro"]
    auc = results["auc_macro"]

    print(f"[{model_name.upper()}] F1={f1:.4f}, AURC={auc:.4f}")
    ...
    plt.ylabel("True")
    plt.show()
    return f1, auc, y_true, y_pred, clf
...
if __name__ == "__main__":
    ...
    # === 阶段1：分解 ===
    #step1_vmd_decompose(data_root, vmd_save_dir="outputs/imfs_5", single_file=None)

    # === 阶段2：特征提取 ===
    # step2_extract_features(vmd_save_dir="outputs/imfs_5", feat_save_dir="outputs/features_5")

    # === 阶段3：分类 ===
    step3_train_classifier(feature_dir="outputs/fused_features/test_clean", model_name="rf")
